...
def corr_last_vs_pred(pred: np.ndarray, last: np.ndarray) -> float:
    ...
    pred: וקטור תחזית (features,)
    last: הדגימה האחרונה ב-segment (features,)
    """
...
def corr_pred_vs_true(pred: np.ndarray, true: np.ndarray) -> float:
    ...
    pred: וקטור תחזית (features,)
    true: הדגימה האמיתית באותו אופק (features,)
    """
    return pearsonr(pred.ravel(), true.ravel())[0]


# ————————————————————————————————————————————————
# 2) מימוש PositionalEncoder ו־Transformer כפי במקור
# ————————————————————————————————————————————————
...
# ————————————————————————————————————————————————
# 3) הגדרת נתיבים, פרמטרים וטעינת מודלים
# ————————————————————————————————————————————————
...
# ————————————————————————————————————————————————
# 4) לולאת חישוב קורלציות וציור גרף משולב
# ————————————————————————————————————————————————
for subj in subjects:
    subj_dir = os.path.join(data_dir, subj)
    pkl_files = sorted(f for f in os.listdir(subj_dir) if f.endswith('.pkl'))

    cors_last = {h: [] for h in range(1, horizons+1)}
    cors_true = {h: [] for h in range(1, horizons+1)}
    ...
    for fname in pkl_files:
        ...
        for h, m in enumerate(models, start=1):
            ...
            # קורלציה בין החיזוי לדגימה האחרונה בחלון
            cors_last[h].append(corr_last_vs_pred(pred, last_sample))

            # קורלציה בין החיזוי לדגימה האמיתית באופק h
            true_sample = data[window_start + window_size + (h-1), :]
            cors_true[h].append(corr_pred_vs_true(pred, true_sample))

    horizons_sorted = sorted(cors_last.keys())
    mean_last = [np.mean(cors_last[h]) for h in horizons_sorted]
    mean_true = [np.mean(cors_true[h]) for h in horizons_sorted]
