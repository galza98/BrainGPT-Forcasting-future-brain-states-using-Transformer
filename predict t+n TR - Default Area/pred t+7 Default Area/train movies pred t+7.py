...
for epoch in range(epochs):
    ...
    if np.mean(test_mse) < best_val_loss:
        ...
        torch.save(model, f"predict_TR+7_transformer_Default_AREA.pth")
    else:
        epochs_no_improve += 1
...
torch.save(model, f"predict_TR+7_transformer_Default_AREA.pth")
np.save(f'predict_TR+7_transformer_Default_AREA_train_loss.npy', loss_hist)
np.save(f'predict_TR+7_transformer_Default_AREA_valid_loss.npy', val_loss_hist)
...
plt.legend()
plt.savefig(f'predict_TR+7_transformer_Default_AREA.png')
plt.clf()
plt.show()