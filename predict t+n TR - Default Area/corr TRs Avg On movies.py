...
# -----------------------------------------------------------------------------
# 3) טוענים מודל יחיד (state_dict) ומוודאים float32
# -----------------------------------------------------------------------------
...
horizons = len(models)  # 7

# צורות סימון שונות לכל אופק (ללא תלות בצבע)
markers = ['o', 's', '^', 'D', 'v', 'P', '*']
...
# -----------------------------------------------------------------------------
# 5) עבור כל נבדק: חשב ממוצע קורלציה לכל אופק וצר גרף
# -----------------------------------------------------------------------------
for subj in subjects:
    # קבצי הבדיקה של הנבדק
    subj_dir = os.path.join(data_dir, subj)
    pkl_files = sorted(f for f in os.listdir(subj_dir) if f.endswith('.pkl'))

    # אגירת קורלציות לכל אופק
    cors = { h: [] for h in range(1, horizons+1) }

    # חשב r לכל קובץ בכל אופק
    for fname in pkl_files:
        arr = np.array(pickle.load(open(os.path.join(subj_dir, fname),'rb')))
        # וידוא that arr is (T,47)
        ...
        enc_in = torch.tensor(segment, device=device, dtype=torch.float32).unsqueeze(0)

        for h, m in enumerate(models, start=1):
            last = segment[-1, :]
            dec_in = torch.tensor(last, device=device, dtype=torch.float32) \
                        .unsqueeze(0).unsqueeze(1)
            print(f"Predicting T+{h+1}")
            print(f"h = {h}")
            print(f"TR Num Is  = {window_start + window_size + (h)}")

            with torch.no_grad():
                pred = m(enc_in, dec_in).cpu().numpy().squeeze()
            true = data[window_start + window_size + (h), :]

            r = pearsonr(pred.ravel(), true.ravel())[0]
            cors[h].append(r)

    # ממוצע לכל אופק
    horizons_sorted = sorted(cors.keys())
    mean_rs = [np.mean(cors[h]) for h in horizons_sorted]

    # ציור גרף
    plt.figure(figsize=(6,4))
    # קו שחור ואחיד
    plt.plot(horizons_sorted, mean_rs, linestyle='-', marker='None')
    # סימון נקודות עם צורות שונות
    for i, (h, r) in enumerate(zip(horizons_sorted, mean_rs)):
        plt.scatter(h, r, marker=markers[i], s=100)
        plt.text(h, r+0.01, f"{r:.2f}", ha='center', va='bottom', fontsize=8)

    plt.title(f"Subject {subj} — Mean Pearson r per Horizon")
