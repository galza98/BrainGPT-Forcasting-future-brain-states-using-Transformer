    ...
    def __init__(self, data_dir, sub_list, sample_size=30, device='cuda' if torch.cuda.is_available() else 'cpu'):
        ...
        for sub in sub_list:
            ...
            for pkl_file in pkl_files:
                ...
                num_samples = data.shape[0] - sample_size - 9  # 110
                for i in range(num_samples):
                    window = data[i:i + sample_size, :]  # [30, 300] ; TRi - TRi + 29
                    target = data[i + sample_size + 9, :]  # [300] ; TR31
...
for epoch in range(epochs):
    ...
    if np.mean(test_mse) < best_val_loss:
        ...
        torch.save(model, f"predict_TR+10_transformer_All_Brain.pth")
    else:
        epochs_no_improve += 1
...
torch.save(model, f"predict_TR+10_transformer_All_Brain.pth")
np.save(f'predict_TR+10_transformer_All_Brain_train_loss.npy', loss_hist)
np.save(f'predict_TR+10_transformer_All_Brain_valid_loss.npy', val_loss_hist)
...
plt.legend()
plt.savefig(f'predict_TR+10_transformer_All_Brain.png')
plt.clf()
plt.show()