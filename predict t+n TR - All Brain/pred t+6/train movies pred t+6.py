...
for epoch in range(epochs):
    ...
    if np.mean(test_mse) < best_val_loss:
        ...
        #save_dir = r"D:\Final Project\Yuval_Gal\brain_state_pred-main\נסיונות\VIS_ZSCORE_VS_NO_ZSCORE\Results"
        #os.makedirs(save_dir, exist_ok=True)
        #model_path = os.path.join(save_dir, f"Small_NO_ZS_transformer_VIS_AREA.pth")
        torch.save(model, f"predict_TR+6_transformer_All_Brain.pth")
    else:
        epochs_no_improve += 1
...
torch.save(model, f"predict_TR+6_transformer_All_Brain.pth")
np.save(f'predict_TR+6_transformer_All_Brain_train_loss.npy', loss_hist)
np.save(f'predict_TR+6_transformer_All_Brain_valid_loss.npy', val_loss_hist)

x_values = list(range(1, len(loss_hist)))
...
plt.legend()
plt.savefig(f'predict_TR+6_transformer_All_Brain.png')
plt.clf()
plt.show()