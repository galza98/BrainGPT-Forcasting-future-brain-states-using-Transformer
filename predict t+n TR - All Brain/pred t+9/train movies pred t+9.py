...
for epoch in range(epochs):
    ...
    if np.mean(test_mse) < best_val_loss:
        ...
        torch.save(model, f"predict_TR+9_transformer_All_Brain.pth")
    else:
        epochs_no_improve += 1
...
torch.save(model, f"predict_TR+9_transformer_All_Brain.pth")
np.save(f'predict_TR+9_transformer_All_Brain_train_loss.npy', loss_hist)
np.save(f'predict_TR+9_transformer_All_Brain_valid_loss.npy', val_loss_hist)
...
plt.legend()
plt.savefig(f'predict_TR+9_transformer_All_Brain.png')
plt.clf()
plt.show()