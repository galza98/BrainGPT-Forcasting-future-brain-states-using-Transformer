...
for epoch in range(epochs):
    ...
    if np.mean(test_mse) < best_val_loss:
        ...
        torch.save(model, f"predict_TR+3_transformer_VIS_AREA.pth")
    else:
        epochs_no_improve += 1
...
torch.save(model, f"predict_TR+3_transformer_VIS_AREA.pth")
# torch.save(model, f'predict_TR+2_transformer_epo-{epochs}_win-{window_size}.pth')
np.save(f'predict_TR+3_transformer_VIS_AREA_train_loss.npy', loss_hist)
np.save(f'predict_TR+3_transformer_VIS_AREA_valid_loss.npy', val_loss_hist)
